...
Anchored to the VERIFIED FEMM baseline (config.M0_UH at BASELINE_TURNS turns
on BOTH coils, solved time-harmonic at config.FREQUENCY_HZ). The old version
anchored to a stale DC value and divided by Ns0=60 even though the FEMM model
has 100 turns on both coils, inflating every M by 1.67x.
...
verified by the FEMM property sweep.
"""
import os
import numpy as np
import pandas as pd
...
for turns in turns_list:
    for scale in area_scale_list:
        ...
        # Required voltage at resonance to push 5A (only R_ac remains)
        lp = turns * 2 * np.pi * rp
