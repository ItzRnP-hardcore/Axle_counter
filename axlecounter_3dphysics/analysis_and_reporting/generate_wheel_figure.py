"""Plot the FEMM flux map for the wheel-present case.

Reads the solved scratch model left behind by femm_wheel_dip.py
(_wheel_work.fem geometry + _wheel_work.ans solution) and renders the field
with the steel wheel flange drawn on top, so the shielding effect of the
wheel on the TX->RX flux path is visible.

Writes reports/figures/11_flux_with_wheel.png.
"""
import sys, os
...
# Project root comes from config -- this script lives in a subfolder, so
# dirname(__file__) is NOT where femm_wheel_dip.py leaves its scratch files.
...
_fem=os.path.join(config.BASE_DIR,"_wheel_work.fem")
_ans=os.path.join(config.BASE_DIR,"_wheel_work.ans")
...
# Segment group -> colour. In the saved model: group 1 = LEFT coil = RX
# ("Receiver"), group 2 = RIGHT coil = TX ("New Circuit"). Group 0 is the
# rail and outer boundary.
gcol={0:"#555555",1:ACCENT3,2:ACCENT2,3:"#7f8c8d"}

# Filled colour = Az magnitude (uWb/m); the overlaid black contour lines are
# lines of constant Az, i.e. magnetic flux lines.
...
# Draw each geometry edge on top of the field, coloured by its group.
for n0,n1,g in segs:
...
# Overlay the wheel flange footprint. The rectangle is drawn from the same
# config constants femm_wheel_dip.py used to place the steel block, so the
# annotation always matches the solved geometry.
