...
void dsp_processing_task(void *parameter) {
  ...
  while (1) {
    ...
    if (ret == ESP_OK) {
      ...
      // Timeout check: Reset and pause if no wheels are detected for TIMEOUT_MS
      unsigned long now = pdTICKS_TO_MS(xTaskGetTickCount());
      if (total_wheels_detected > 0 &&
        ...
        ESP_LOGI(TAG, "Pausing detection for 2 seconds to reset state...");

        // Pause ADC continuous detection
        ...
        // Flush any pending task notification triggers that queued up during
        // delay
        ulTaskNotifyTake(pdTRUE, 0);

        ESP_LOGI(TAG, "Resuming detection with a clean slate!");
        last_wheel_detection_time =
            pdTICKS_TO_MS(xTaskGetTickCount()); // Reset timestamp to now

        // Resume ADC continuous detection
