...
const float MAG_THRESHOLD =
    80.0f; // Adjusted for WROOM noise floor and peak detection

#define TIMEOUT_MS 20000 // 20 Seconds in milliseconds
...
__attribute__((aligned(16))) float v1_complex[2048];
__attribute__((aligned(16))) float v2_complex[2048];
__attribute__((aligned(16))) float wind_hann[1024];

unsigned long tS1 = 0, tS2 = 0;
// Start disarmed. The system MUST see a high EM signal to arm itself before
// counting.
bool s1_Armed = false, s2_Armed = false;
unsigned long last_wheel_detection_time = 0;
unsigned long ignore_detections_until =
    0; // Timestamp to ignore false readings during stabilization
...
const float DIP_THRESHOLD = 400.0f; // Signal drops below this when wheel passes
const float ARM_THRESHOLD =
    500.0f; // EM wave must rise above this to arm sensor

void process_axle_logic(float m1, float m2) {
  unsigned long now = pdTICKS_TO_MS(xTaskGetTickCount());

  // Skip logic if we are within the initialization/stabilization delay
  ...
  if (m1 > ARM_THRESHOLD) {
    s1_Armed = true; // Signal is high (EM wave present). Sensor is ARMED!
  } else if (m1 < DIP_THRESHOLD && s1_Armed) {
    ESP_LOGI(TAG, "Sensor 1 DIP Detected!");
    tS1 = now;
    s1_Armed = false; // Triggered! Disarm until signal returns.
  }

  // --- SENSOR 2 STATE MACHINE ---
  if (m2 > ARM_THRESHOLD) {
    s2_Armed = true; // Signal is high (EM wave present). Sensor is ARMED!
  } else if (m2 < DIP_THRESHOLD && s2_Armed) {
    ESP_LOGI(TAG, "Sensor 2 DIP Detected!");
    tS2 = now;
    s2_Armed = false; // Triggered! Disarm until signal returns.
  }
  ...
  if (tS1 > 0 && tS2 > 0) {
    ...
    last_wheel_detection_time = now; // Update last wheel detection time
  ...
  while (1) {
    ...
    if (ret == ESP_OK) {
      ...
      if (log_divider++ % 2 == 0) {
        ...
        unsigned long now = pdTICKS_TO_MS(xTaskGetTickCount());
        ESP_LOGI("GRAPH", "T:%4ld | W:%2d | S1:%6.1f %-40s | S2:%6.1f %-40s",
                 now, total_wheels_detected, magS1, bar1, magS2, bar2);
      }

      process_axle_logic(magS1, magS2);

      // Timeout check: Reset state if no wheels are detected for TIMEOUT_MS
      unsigned long now = pdTICKS_TO_MS(xTaskGetTickCount());
      if (total_wheels_detected > 0 &&
          (now - last_wheel_detection_time >= TIMEOUT_MS)) {
        ESP_LOGI(TAG,
                 "=== TIMEOUT REACHED (No wheels detected for %d seconds) ===",
                 TIMEOUT_MS / 1000);
        ESP_LOGI(TAG, "Final Net Wheels Crossed: %d", total_wheels_detected);
        ESP_LOGI(TAG, "Resetting slate to start fresh...");

        // CRITICAL FIX: We MUST NOT call adc_continuous_stop() here!
        // Stopping the ADC from inside its own processing task causes a
        // FreeRTOS mutex panic (xTaskPriorityDisinherit). We just reset
        // variables instantly.
        ...
        // Disarm the system! It must see the EM signal again to re-arm.
        s1_Armed = false;
        s2_Armed = false;

        last_wheel_detection_time =
            pdTICKS_TO_MS(xTaskGetTickCount()); // Reset timestamp to now
      }
    }
  }
}
...
void app_main(void) {
  ...
  // Set the boot stabilization window (ignore detections for the first 2
  // seconds)
  ignore_detections_until = pdTICKS_TO_MS(xTaskGetTickCount()) + 2000;
